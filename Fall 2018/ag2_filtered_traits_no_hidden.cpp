...
int main()
{
  std::ifstream ifs("data/new_sites.cin");
  assert( ifs );
  ...
  std::ofstream file("data/new_output.txt");
  ag.file_output(file);
/*
  for(Apollonius_graph::Finite_vertices_iterator vi = ag.finite_vertices_begin(); vi != ag.finite_vertices_end();vi++){
	std::cout << vi.Point()->x() << std::endl;
  }

  for(tm = ag.finite_vertices_begin();tm!=ag.finite_vertices_end();tm++){
	std::cout << tm->point();
  }
*/
  std::cout << ag.number_of_vertices()  << std::endl;
